    ...
    try {
        ...
        if (window.allDashProjects) {
            window.allDashProjects.forEach(function(p) { 
                // 출하 예정일은 연도 추출에서 제외, 오직 출하 완료일만 기준
        ...
        if (window.allDashProjects) {
            window.allDashProjects.forEach(function(data) {
                ...
                let isInYear = false;
                // 해당 연도의 출하 완료일이 있거나, 아직 진행/대기 중인 경우 포함 (예정일 제외)
                ...
                // 오직 출하 완료일이 해당 연도/월일 때만 완료 건수로 집계
                ...
                // 지연/위험 여부는 기준일(예정일) 대비 지났는지를 봐야하므로 예정일 사용
...
### 2. `js/project.js` (현황판 미니 대시보드 로직 수정)
현황판 상단 미니 대시보드에서도 "출하 예정일"이 카운트에 영향을 주지 않고, **순수하게 "출하 완료일"이 해당 달(Month)인 경우에만 [출하 완료] 뱃지가 증가**하도록 반영했습니다.

```javascript
/* eslint-disable */
import { db } from './firebase.js';
import { collection, doc, setDoc, getDoc, getDocs, addDoc, deleteDoc, query, onSnapshot, where, serverTimestamp, writeBatch } from '[https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js](https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js)';
...
window.getFilteredProjects = function() {
    ...
    if(window.currentMonthFilter) { 
        list = list.filter(item => { 
            ...
            // 출하 예정일(sEst)는 리스트 필터 기준에서도 삭제 (출하 완료일 기준)
    ...
    try {
        ...
        (window.currentProjectStatusList || []).forEach(function(item) {
            ...
            if (status === 'pending' || status === 'rejected') {
                ...
                // [변경점] 출하예정일(shipEst) 백업 로직 제거, 오직 출하완료일(shipEn)만 기준
