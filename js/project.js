...
// 💡 유틸리티: 이미지 첨부 시 용량 최적화 및 Base64 변환 (에러 완전 차단)
    ...
    currentPurchaseUnsubscribe = onSnapshot(query(collection(db, "project_purchases"), where("projectId", "==", projectId)), function(snap) { 
        ...
        listEl.innerHTML = list.map(item => {
            ...
            return `<div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-2">
                        ...
                        ${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank" class="text-xs text-sky-500 font-bold underline mt-1 w-fit"><i class="fa-solid fa-paperclip"></i> 첨부 일정표 확인하기</a>` : ''}
                    </div>`;
        }).join('');
    });
};
    ...
    currentDesignUnsubscribe = onSnapshot(query(collection(db, "project_designs"), where("projectId", "==", projectId)), function(snap) { 
        ...
        listEl.innerHTML = list.map(item => {
            ...
            return `<div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-2">
                        ...
                        <div class="text-[13px] text-slate-700 font-medium break-words leading-relaxed">${safeContent}</div>
                        ${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank" class="text-xs text-sky-500 font-bold underline mt-1 w-fit"><i class="fa-solid fa-file-arrow-down"></i> 설계 파일 확인하기</a>` : ''}
                    </div>`;
        }).join('');
    });
};
    ...
    currentPjtScheduleUnsubscribe = onSnapshot(query(collection(db, "project_schedules"), where("projectId", "==", projectId)), function(snap) { 
        ...
        listEl.innerHTML = list.map(item => {
            ...
            return `<div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-2">
                        ...
                        <div class="text-[13px] text-slate-700 font-medium break-words leading-relaxed">${safeContent}</div>
                        ${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank" class="text-xs text-sky-500 font-bold underline mt-1 w-fit"><i class="fa-solid fa-calendar-days"></i> PJT 일정표 확인하기</a>` : ''}
                    </div>`;
        }).join('');
    });
};
    ...
    try {
        ...
        logs.forEach(function(log) { 
            ...
            listHtml += `<div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-1 hover:shadow-md transition-shadow">
                            ...
                            <div class="text-slate-700 font-medium text-[13px] pl-1 mt-2 break-words leading-relaxed">${safeContent}</div>
                            ${imgHtml}
                        </div>`; 
        });
        ...
        try { 
            ...
            if (proj && proj.manager && proj.manager !== window.userProfile.name && (!notified || !notified.includes(proj.manager))) {
                if(window.notifyUser) {
                    await window.notifyUser(proj.manager, content, projectId, "새 생산일지");
                }
            }
...
window.renderLinksList = function(projectId) { 
    try {
        ...
        let htmlStr = ''; proj.links.forEach(function(lnk, idx) { htmlStr += `<li class="flex justify-between items-center p-4 bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow group"><div class="flex flex-col truncate"><span class="font-extrabold text-slate-700 text-sm mb-1">${getSafeString(lnk.name)}</span><a href="${getSafeString(lnk.url)}" target="_blank" class="text-[11px] font-bold text-teal-600 hover:text-teal-800 hover:underline truncate flex items-center gap-1"><i class="fa-solid fa-link"></i> ${getSafeString(lnk.url)}</a></div><button onclick="window.deleteLinkItem('${projectId}', ${idx})" class="text-slate-300 hover:bg-rose-50 hover:text-rose-500 rounded-lg transition-all p-2.5"><i class="fa-solid fa-trash-can"></i></button></li>`; }); list.innerHTML = htmlStr;
    } catch(e) {}
};
...
window.showAutocomplete = function(inputEl, targetId1, targetId2, isNameSearch) {
    ...
    for (let i = 0; i < window.pjtCodeMasterList.length; i++) {
        ...
        if (isNameSearch) { if (p.name.toLowerCase().includes(val) || window.matchString(val, p.name)) matches.push(p); } 
        else { if (p.code.toLowerCase().includes(val)) matches.push(p); }
    }
    if(matches.length > 0) {
        ...
        matches.forEach(function(m) {
            let safeCompany = m.company || '업체미상'; let safeName = m.name.replace(/'/g, "&#39;").replace(/"/g, "&quot;");
...
window.loadNcrData = async function() {
    try {
        ...
        // CSV 방식은 CORS 에러가 나므로, 구글이 허용하는 유일한 경로인 JSONP 방식(/gviz/tq)을 사용합니다!
        const url = `https://docs.google.com/spreadsheets/d/${sheetId}/gviz/tq?tqx=out:json&gid=0`;

        const res = await fetch(url);
        if (!res.ok) throw new Error("구글 시트를 읽을 수 없습니다.");

        const text = await res.text();
        
        // 💡 에러 원인 해결: 정규식을 버리고 가장 안전한 텍스트 자르기(substring) 방식 적용!
        ...
        // 헤더(첫 줄)가 비어있으면 데이터의 첫 줄을 헤더로 삼습니다.
