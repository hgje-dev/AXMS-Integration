...
import { collection, doc, setDoc, onSnapshot, query, orderBy } from 'https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js';

// 💡 1. PJT 현황 전용 구글 드라이브 폴더 ID
    ...
    <div class="space-y-6">
        <div class="flex justify-between items-center bg-white p-6 rounded-3xl border shadow-sm">
            <div>
                ...
                <p class="text-xs text-slate-400 font-bold mt-1 uppercase tracking-wider">Project Lifecycle Management</p>
            </div>
            ...
            <table class="w-full text-left border-collapse min-w-[1200px]">
                <thead class="bg-slate-800 text-white text-[11px] uppercase tracking-widest">
                    <tr>
                        ...
                        <th class="p-4 w-32 text-center text-blue-300">구매</th>
                        <th class="p-4 w-32 text-center text-emerald-300">설계</th>
                        <th class="p-4 w-32 text-center text-amber-300">일정</th>
                        <th class="p-4">생산일지(진행현황)</th>
                        <th class="p-4 text-center w-28">최종업데이트</th>
                    </tr>
                </thead>
                <tbody id="pjt-list-tbody" class="divide-y text-xs">
                    </tbody>
            </table>
        </div>
    </div>`;
            ...
            <tr class="hover:bg-slate-50 cursor-pointer transition-colors border-b" onclick="window.openProjectDetailModal('${id}')">
                ...
                <td class="p-4 text-center">
                    <div class="flex flex-col gap-1 items-center">
                        <span class="truncate w-24 text-[10px] text-slate-500">${p.buyText || '-'}</span>
                        <i class="fa-solid fa-file-invoice ${buyIcon} text-lg"></i>
                    </div>
                </td>
                <td class="p-4 text-center">
                    <div class="flex flex-col gap-1 items-center">
                        <span class="text-[10px] font-bold text-slate-400">${p.designFileUrl ? '도면첨부' : '-'}</span>
                        <i class="fa-solid fa-drafting-compass ${designIcon} text-lg"></i>
                    </div>
                </td>
                <td class="p-4 text-center">
                    <div class="flex flex-col gap-1 items-center">
                        <span class="truncate w-24 text-[10px] text-slate-500">${p.schText || '-'}</span>
                        <i class="fa-solid fa-calendar-days ${schIcon} text-lg"></i>
                    </div>
                </td>
                <td class="p-4 text-slate-600 leading-relaxed font-medium">
                    ${(p.statusText || '기록된 내용이 없습니다.').substring(0, 100)}...
                </td>
...
window.openProjectDetailModal = async function(id) {
    const pD = window.allProjects ? window.allProjects.find(x => x.id === id) : null;
    // (PJT 목록 캐싱이 없다면 Firestore에서 직접 단건 조회를 수행하도록 구현)
    // 여기서는 편의상 전역 변수나 snapshot에서 찾음
    
    // 모달 데이터 채우기
    const docSnap = await getDocs(query(collection(db, "projects"))); // 실제 환경에서는 id로 직접 getDoc 하세요
    let p = null;
    docSnap.forEach(d => { if(d.id === id) p = d.data(); });
    ...
    if(p.buyFileUrl) { bL.innerHTML = `<a href="${p.buyFileUrl}" target="_blank" class="hover:underline">📎 첨부파일 보기</a>`; bL.classList.remove('hidden'); }
    else { bL.classList.add('hidden'); }

    const dL = document.getElementById('pjt-design-file-link');
    if(p.designFileUrl) { dL.innerHTML = `<a href="${p.designFileUrl}" target="_blank" class="hover:underline">📎 설계도면 보기</a>`; dL.classList.remove('hidden'); }
    else { dL.classList.add('hidden'); }

    const sL = document.getElementById('pjt-sch-file-link');
    if(p.schFileUrl) { sL.innerHTML = `<a href="${p.schFileUrl}" target="_blank" class="hover:underline">📎 일정표 보기</a>`; sL.classList.remove('hidden'); }
...
window.saveProjectDetail = async function() {
    ...
    window.showToast("저장 중...");
    
    try {
        let buyUrl = null, designUrl = null, schUrl = null;

        // 구글 드라이브 업로드 (window.uploadFileToDrive 함수가 request.js 등에 구현되어 있어야 함)
        ...
        if(buyF) {
            const fId = await window.uploadFileToDrive(buyF, PJT_STATUS_FOLDER_ID);
            buyUrl = `https://drive.google.com/file/d/${fId}/view`;
        }
        if(designF) {
            const fId = await window.uploadFileToDrive(designF, PJT_STATUS_FOLDER_ID);
            designUrl = `https://drive.google.com/file/d/${fId}/view`;
        }
        if(schF) {
            const fId = await window.uploadFileToDrive(schF, PJT_STATUS_FOLDER_ID);
            schUrl = `https://drive.google.com/file/d/${fId}/view`;
        }
        ...
        if(buyUrl) payload.buyFileUrl = buyUrl;
        if(designUrl) payload.designFileUrl = designUrl;
        if(schUrl) payload.schFileUrl = schUrl;

        await setDoc(doc(db, "projects", id), payload, { merge: true });
        
        window.showToast("프로젝트 현황이 업데이트되었습니다.");
