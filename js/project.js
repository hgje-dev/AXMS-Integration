...
window.deleteAllNotifications = async function() {
    ...
    try {
        ...
        await batch.commit();
        window.showToast("모든 알림이 삭제되었습니다.");
    } catch(e) {}
};
...
window.renderNotifications = function(notifs, unreadCount) {
    ...
    listEl.innerHTML = notifs.map(n => `
        <div class="px-4 py-3 hover:bg-indigo-50/50 cursor-pointer transition-colors border-b border-slate-50 last:border-0 ${n.isRead ? 'opacity-50' : 'bg-white'} relative group" onclick="window.readAndNavigateNotification('${n.id}', '${n.projectId}', '${n.type}')">
            <div class="flex justify-between items-start">
                ...
                <button onclick="window.deleteNotification(event, '${n.id}')" class="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity p-1"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <p class="text-[10px] text-slate-500 pl-3 leading-relaxed">${n.message}</p>
