...
// 🌟 안전한 밀리초(날짜) 변환 함수: 과거 숫자형 데이터와 최신 Timestamp 데이터를 모두 안전하게 처리 (에러 원인 1 완벽 차단)
...
window.loadCounts = function() {
    onSnapshot(collection(db, "project_comments"), snap => {
        ...
        if(window.currentProjDashView === 'list' && document.getElementById('view-project-status').classList.contains('hidden') === false) window.renderProjectStatusList();
    });
...
window.loadProjectStatusData = function() {
    ...
    projectStatusSnapshotUnsubscribe = onSnapshot(query(collection(db, "projects_status")), (snapshot) => {
        ...
        snapshot.forEach(doc => { 
            ...
            // 🚨 핵심 픽스: 과거 데이터(part 필드 없는 데이터)를 투명인간 취급하지 않게 기본 '제조' 파트로 강제 설정! (에러 원인 2 차단)
...
window.renderProjCodeMasterList = function() { const tbody = document.getElementById('pjt-code-tbody'); if(!tbody) return; if(window.pjtCodeMasterList.length === 0) { tbody.innerHTML = `<tr><td colspan="4" class="text-center p-6 text-slate-400 font-bold">등록된 PJT 코드가 없습니다.</td></tr>`; return; } tbody.innerHTML = window.pjtCodeMasterList.map(c => `<tr class="hover:bg-slate-50 transition-colors"><td class="p-3 font-bold text-indigo-600">${c.code}</td><td class="p-3 font-bold text-slate-700">${c.name}</td><td class="p-3 text-slate-600">${c.company}</td><td class="p-3 text-center"><button onclick="window.deleteProjectCode('${c.id}')" class="text-slate-400 hover:text-rose-500 p-1"><i class="fa-solid fa-trash-can"></i></button></td></tr>`).join(''); };
...
window.showAutocomplete = function(inputEl, nameId, compId, isNameSearch=false) { let listId = inputEl.id + '-autocomplete-list'; let listEl = document.getElementById(listId); if(!listEl) { listEl = document.createElement('div'); listEl.id = listId; listEl.className = "absolute z-50 w-[150%] max-w-[400px] bg-white border border-slate-200 shadow-2xl rounded-xl mt-1 hidden max-h-60 overflow-y-auto custom-scrollbar"; inputEl.parentNode.style.position = 'relative'; inputEl.parentNode.appendChild(listEl); document.addEventListener('click', (e) => { if(e.target !== inputEl && !listEl.contains(e.target)) listEl.classList.add('hidden'); }); } const queryStr = inputEl.value; let matches = window.pjtCodeMasterList; if(queryStr.trim() !== '') matches = window.pjtCodeMasterList.filter(c => window.matchString(queryStr, c.code) || window.matchString(queryStr, c.name) || window.matchString(queryStr, c.company)); matches = matches.slice(0, 100); if(matches.length === 0) { listEl.innerHTML = `<div class="p-3 text-xs text-slate-400 text-center font-bold">검색 결과 없음<br><span class="text-[9px] font-normal">(직접 입력하여 사용 가능)</span></div>`; } else { listEl.innerHTML = matches.map(c => `<div class="p-3 text-xs hover:bg-indigo-50 cursor-pointer border-b border-slate-100 last:border-0 transition-colors" onclick="window.selectAutocomplete('${inputEl.id}', '${nameId}', '${compId}', '${c.code}', '${c.name}', '${c.company}', ${isNameSearch})"><div class="font-black text-indigo-600 mb-0.5">${c.code}</div><div class="text-slate-700 font-bold truncate">${c.name} <span class="text-slate-400 font-medium ml-1">(${c.company})</span></div></div>`).join(''); } listEl.classList.remove('hidden'); };
