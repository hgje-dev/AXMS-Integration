...
// ==========================================
// 🔐 권한 체크 헬퍼 함수 (request.js 자체 내장)
// ==========================================
function checkReqPermission(action) {
    if (!window.userProfile) return false;
    // 시스템 관리자는 무조건 허용
    if (window.userProfile.role === 'admin' || window.userProfile.role === 'sys_admin') return true;
    
    // 개인별 예외 권한(custom_permissions)이 있다면 최우선 적용
    if (window.userProfile.custom_permissions && typeof window.userProfile.custom_permissions[action] === 'boolean') {
        return window.userProfile.custom_permissions[action];
    }
    
    // 직책별 기본 권한
    const role = window.userProfile.role || 'member';
    if (action === 'req_approve') {
        // 팀장, 본부장, 관리자급은 승인(Approve) 기본 허용
        return ['team_leader', 'part_leader', 'division_leader'].includes(role);
    }
    if (action === 'req_write') {
        // 대표(ceo)를 제외한 실무자는 기본 작성 허용
        return role !== 'ceo';
    }
    return false;
}
...
const ADMIN_EMAIL = 'mfg@axbis.ai'; // 기본 수신 관리자 이메일
...
window.openWriteModal = function(editId = null) { 
    // 💡 권한 체크: 신규 작성이면 req_write 권한이 필요
    if (!editId && !checkReqPermission('req_write')) {
        return window.showToast("요청서 작성 권한이 없습니다.", "error");
    }
    ...
    if (editId) {
        ...
        if (req) {
            ...
            // 💡 권한 체크: '요청서 결재/승인(req_approve)' 권한이 있는 사람만 접수/완료 버튼 노출
            if (checkReqPermission('req_approve')) {
            ...
            // 💡 수정 권한 제어: 본인 글이 아니면 저장 버튼 숨기기 (단, 승인권자는 저장 가능)
            const saveBtn = document.getElementById('btn-req-save');
            if (req.authorUid !== window.currentUser.uid && !checkReqPermission('req_approve')) {
...
window.acceptRequest = async function() {
    ...
    if (!checkReqPermission('req_approve')) return window.showToast("권한이 없습니다.", "error");
...
window.renderRequestList = function() { 
    ...
    tb.innerHTML = window.currentRequestList.map(r=> {
        ...
        // 💡 본인이 작성했거나 승인 권한이 있는 사람만 삭제 버튼 노출
        let deleteBtn = '';
        if (r.authorUid === window.currentUser.uid || checkReqPermission('req_approve')) {
            deleteBtn = `<button onclick="window.deleteRequest('${r.id}')" class="text-slate-300 hover:text-rose-500 transition-colors p-2"><i class="fa-solid fa-trash-can"></i></button>`;
        }
