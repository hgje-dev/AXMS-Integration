...
window.checkMissingMembers = function() {
    try {
        ...
        // UID 불일치 버그 해결을 위해 '이름' 기반으로 매칭
        const submittedNames = window.currentWeeklyLogList.filter(l => l.isSubmitted).map(l => l.authorName);
        
        let missing = [];
        window.teamMembers.forEach(tm => {
            if(!submittedNames.includes(tm.name) && tm.name !== '시스템관리자') {
                missing.push(tm);
            }
        });
        ...
        if(missing.length > 0) {
            ...
            setText('missing-count', missing.length);
            if(listEl) {
    ...
    } catch(e) { console.error("Missing Members Error:", e); }
};
...
window.renderWeeklyLogs = function() { 
    try {
        ...
        const searchName = getEl('weekly-search-name')?.value.toLowerCase() || '';
        const searchContent = getEl('weekly-search-content')?.value.toLowerCase() || '';
        ...
        if(searchContent) {
            displayList = displayList.filter(l => {
                ...
                (l.tasks || []).forEach(t => fullText += ' ' + String(t.content || ''));
        ...
        feed.innerHTML = displayList.map(log => {
            ...
            if(safeIssuesStr !== '') {
                ...
                issuesHtml = `<div class="mt-5 bg-[#fffbf0] border border-orange-100 rounded-xl p-4 shadow-sm">
                    ...
                    <div class="text-sm font-medium text-slate-700 leading-relaxed">${formattedIssue}</div>
                </div>`;
            }
    ...
    } catch(e) { console.error("Render Logs Error:", e); }
};
...
window.openWeeklyLogWriteModal = function(editId = null) { 
    try {
        const weekInput = getEl('weekly-log-filter-week');
        if (!weekInput) return window.showToast("오류: HTML 파일이 최신버전이 아닙니다.", "error");
        ...
        setVal('weekly-draft-id', existingLog ? existingLog.id : '');
        
        window.draftTasks = existingLog && existingLog.tasks ? existingLog.tasks.map((t, i) => ({...t, id: t.id || Date.now() + i})) : [];
        
        setVal('wl-issues', existingLog ? (existingLog.issues || '') : '');
        setVal('wl-pjt-name', existingLog ? (existingLog.projectName || '') : '');
        setVal('wl-pjt-code', existingLog ? (existingLog.projectCode || '') : '');
        setVal('wl-time-h', existingLog ? (existingLog.totalHours || '') : '');
        setVal('wl-time-m', existingLog ? (existingLog.totalMins || '') : '');
...
window.addWeeklyTaskRow = function() {
    try {
        ...
        if(!contentEl) return window.showToast("입력창을 찾을 수 없습니다.", "error");
...
window.renderDraftTasks = function() {
    try {
        const listEl = getEl('wl-task-list');
        if(!listEl) return;
...
window.saveWeeklyLog = async function(isFinalSubmit) { 
    try {
        ...
        if(!draftIdEl || !weekEl) return window.showToast("오류: HTML 로딩이 완료되지 않았습니다.", "error");
        ...
        const codeEl = getEl('wl-pjt-code');
        const nameEl = getEl('wl-pjt-name');
...
window.renderKanbanBoard = function() {
        ...
        board.innerHTML = days.map(day => {
            ...
            let eventsHtml = events.map(s => {
                ...
                return `<div class="rounded-xl border p-3 ${style.bg} relative group cursor-pointer hover:shadow-md transition-all" onclick="window.editSchedule('${s.id}')">
                    ...
                    <div class="text-sm font-bold ${style.text} mb-1">${safeContent}</div>
                    <div class="text-[10px] text-slate-500 font-bold flex items-center gap-1"><i class="fa-regular fa-clock"></i> ${safeTime}</div>
                </div>`;
            }).join('');
    ...
    } catch(e) { console.error("Render Kanban Error:", e); }
};
