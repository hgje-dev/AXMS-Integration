...
// 💡 [핵심] 구글 권한 에러 해결 및 공유드라이브 지원 (supportsAllDrives 추가)
window.saveWorkhoursToDrive = async function() {
    ...
    if (!window.googleAccessToken || !storedExpiry || Date.now() > parseInt(storedExpiry)) {
        ...
        return window.showToast("구글 보안 토큰이 만료되었습니다. 로그아웃 후 다시 연동해주세요.", "error");
    }
    ...
    try {
        ...
        if (folderData.files && folderData.files.length > 0) {
            ...
            const patchRes = await fetch(`https://www.googleapis.com/upload/drive/v3/files/${fileId}?uploadType=multipart&supportsAllDrives=true`, {
                method: 'PATCH',
                headers: { 'Authorization': 'Bearer ' + window.googleAccessToken },
                body: form
            });
            ...
            const postRes = await fetch(`https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&supportsAllDrives=true`, {
                method: 'POST',
                headers: { 'Authorization': 'Bearer ' + window.googleAccessToken },
                body: form
            });
        ...
        if (e.message === "TOKEN_EXPIRED") {
            window.showToast("구글 보안 토큰이 만료되었습니다. 로그아웃 후 다시 연동해주세요.", "error");
        } else {
            window.showToast("드라이브 저장에 실패했습니다: " + e.message, "error");
        }
    }
};
